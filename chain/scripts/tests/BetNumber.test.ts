...
describe("BetNumber should", function () {
  ...
  it("be able to create bet and reveal number", async function () {
    ...
    const setValueTransaction = await betNumber['createBet(string,string,string,uint256,uint256,uint256,uint256,address[],address[],int256[])'](uuid, "name", "desc", localHash, convertCourse(2), deadline, deadline+2*deltaTime, [],[],[], { value: ethers.utils.parseEther("0.5") });
  ...
  it("tell user that there is not enough money to pay him", async function () {
    ...
    const deltaTime = 100;
    const deadline = await latestTimestamp()+deltaTime;
    const setValueTransaction = await betNumber.connect(owner)['createBet(string,string,string,uint256,uint256,uint256,uint256,address[],address[],int256[])'](uuid, "name", "desc", localHash, convertCourse(2), deadline, deadline+2*deltaTime, [],[],[], { value: ethers.utils.parseEther("0.5") });
    await setValueTransaction.wait();
  ...
  it("give user proper amount of money", async function () {
    ...
    const deltaTime = 10;
    const deadline = await latestTimestamp()+deltaTime;
    const setValueTransaction = await betNumber.connect(owner)['createBet(string,string,string,uint256,uint256,uint256,uint256,address[],address[],int256[])'](betId, "name", "desc", localHash, convertCourse(2), deadline, deadline+2*deltaTime, [],[],[], { value: ethers.utils.parseEther("0.5") });
    await setValueTransaction.wait();
    //guess
  ...
  it("give users proper amount of money", async function () {
    ...
    const deltaTime = 100;
    const deadline = await latestTimestamp()+deltaTime;
    const setValueTransaction = await betNumber.connect(owner)['createBet(string,string,string,uint256,uint256,uint256,uint256,address[],address[],int256[])'](betId, "name", "desc", localHash, convertCourse(2), deadline+2*deltaTime, deadline+3*deltaTime, [],[],[], { value: ethers.utils.parseEther("0.5") });
    await setValueTransaction.wait();
    await ethers.provider.send('evm_increaseTime', [deltaTime*2]);
    //guess
    const guessNumber = number;
  ...
  it("Tell that it is too late to finish bet", async function () {
    const betNumber = await setUpContract();
    ...
    const revealNumberTransaction = async () => betNumber.connect(owner).revealNumber(betId, number, nonce)

    expectThrowsAsync(revealNumberTransaction, toVMError("It is too late."))
  });
  ...
  it("Allow users to force finish bet", async function () {
    ...
    await betTransaction2.wait();
    await ethers.provider.send('evm_increaseTime', [deltaTime*2]);    
    const forceTrasaction = await betNumber.connect(guesser2).forceFinishBet(betId)
    await forceTrasaction.wait();
  ...
  it("Tell user that it is too early to force finish", async function () {
    const betNumber = await setUpContract();
    ...
    await betTransaction2.wait();
    // await ethers.provider.send('evm_increaseTime', [deltaTime*2]);    
    const forceTrasaction = async () => betNumber.connect(guesser2).forceFinishBet(betId)

    await expectThrowsAsync(forceTrasaction, toVMError("It is too soon, answearRevealDeadline was not reached."))
  });


  it("not allow to force finish after reveal", async function () {
    const betNumber = await setUpContract();
    ...
    await expectThrowsAsync(forceTrasaction, toVMError("Bet is no longer open"))
  });
  ...
  it("allow to bet on an option and reveal answer with number in options", async function () {
    ...
    const deltaTime = 10;
    const deadline = await latestTimestamp()+deltaTime;
    const setValueTransaction = await betNumber.connect(owner)['createBet(string,string,string,uint256,uint256,uint256,uint256,address[],address[],int256[])'](betId, "name", "desc", localHash, convertCourse(2), deadline+2*deltaTime, deadline+3*deltaTime, [],[],[1,2,3], { value: ethers.utils.parseEther("0.5") });
    await setValueTransaction.wait();
    await ethers.provider.send('evm_increaseTime', [deltaTime*2]);
    //guess
    const guessNumber = number;
  ...
  it("Not allow users to bet on non included options", async function () {
    ...
    const deltaTime = 10;
    const deadline = await latestTimestamp()+deltaTime;
    const setValueTransaction = await betNumber.connect(owner)['createBet(string,string,string,uint256,uint256,uint256,uint256,address[],address[],int256[])'](betId, "name", "desc", localHash, convertCourse(2), deadline+2*deltaTime, deadline+3*deltaTime, [],[],[1,2], { value: ethers.utils.parseEther("0.5") });
    await setValueTransaction.wait();
    await ethers.provider.send('evm_increaseTime', [deltaTime*2]);
    //guess
    const guessNumber = number;
  ...
  it("Pay back all the users after creator revelaed a number not in options", async function () {
    const betNumber = await setUpContract();
